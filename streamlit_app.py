...
@st.cache_data
def load_rates():
    """Loads the attached CSV file dynamically using absolute pathing."""
    ...
    try:
        ...
        st.error(f"⚠️ **rates.csv not found!** Python is looking exactly here: {file_path}")
        return {}
    except Exception as e:
        st.error(f"⚠️ **Error reading rates.csv:** {e}")
        return {}

FEDERAL_RATES_DB = load_rates()
LOCATIONS_LIST = sorted(list(FEDERAL_RATES_DB.keys())) if FEDERAL_RATES_DB else ["Please load rates.csv"]
...
with st.sidebar:
    ...
    # Restored to Open Text for the Origin to allow calculation of freeform flight origins
...
for idx, leg in enumerate(raw_legs_inputs):
    ...
    # Fetch coordinates just for this specific leg dynamically for flight calculations
...
if date_sequencing_valid and origin_geo and FEDERAL_RATES_DB and len(legs_data) == st.session_state["num_legs"]:
    ...
    for idx, leg in enumerate(legs_data):
        ...
        # Car rate scaled slightly higher for foreign/OCONUS deployments
